...
- **Status words on the page and in the advisor summary** (DGS 2026-09-22): `unmet` and
  ...
  `advisorSummary` re-voices each row's own detail as the Why (`whyFor`; no course lists, no bullets, the categories row reduced to group names — DGS 2026-09-23) and takes
...
- **Public-transcript fixtures and the column-mapped parser** (2026-09-26). `tests/fixtures/public-transcripts/<name>.json` are 93 line lists composed from PUBLIC registrar samples, keys and templates (never a real record; `sources.json` says where each came from, `expected.json` what it must read, `public-transcripts-known-failing.json` the ones waiting on a DGS decision — a fixture on that list must still fail, so the list stays honest; take it off when its change lands). `tests/public-transcripts.test.ts` pins them; `tests/public-transcript-rules.test.ts` pins the reasoning on minimal documents. Tools: `node --experimental-strip-types scripts/dev/parse-lines.mts <lines.json>` prints one parse; `scripts/dev/public-status.mts [name]` prints pass/fail with diffs. In `src/transcript/external.ts` the row reader now tries `scanWithMap` first — `readColumnHeader` turns the table's header line into column kinds (`COLUMN_KIND_RES`; a cell no pattern knows is a flag; "Note" is the grade only without another grade column; "Points" before the grade with no credits column is the credits; Theoretical/Practical and Créditos Aula/Trabalho sum to the credits) and the row's tail is matched against the kinds after the title, longest title with fewest blank columns wins, one-letter flags and the rest of a type cell may trail — and falls back to the position-free `scanTokens` when the row does not fit (a decimal or trailing integer left in the title with no credits found is the tell). `readLegend` gives `mapGrade` its `LegendHints`; `CODED_*_TOKENS` map only in grade position (after the credits or right before them), so "CS Trends" stays a title. `readTermLine` replaces the old term regex (ordinals via `ordinalOf`, `academicRange` for BUET-style Level/Term lines, `explicit` marks a header that named a season so a row's own date cannot override it, `ubcStyle` gates "Winter Session"). `rowTermOf` reads per-row term cells; `isoOrEuropeanDate` (day-first, ISO, Romance months) complements parse.ts's US `dateOnLine`, and `dateAfterLabel` takes the date after "Graduation Date:" on a two-date line. Degree abbreviations are case-sensitive (`BACHELORS_ABBR_RE`, `GRAD_ABBR_RE`). The research plan with every gap, sample lines and the needs-download list is in the session scratchpad only (`transcripts/final/PLAN.md`); DECISIONS.md carries the readings and the seven open questions.
- **Whose eighth semester** (2026-09-26, evening; DGS rule). Every §4 deadline counts from `Student.entryTerm` (`ctx.entry`, summer → the following fall); the rule about a Notre Dame MSCSE is therefore a rule about WHICH term that is: a finished MSCSE → the Ph.D. entry (`ndMasters` set, `background.graduate` = `nd-mscse` / `nd-4plus1`); a transfer into the Ph.D. from an unfinished MSCSE → the MSCSE start (`background.graduate` = `nd-mscse-transfer`, a Ph.D.-only answer in `graduateOptions`; `applyBackground` sets no prior degree for it; `state.ts` accepts it). `candidacyRow` in phd.ts appends the “Semesters are counted from …” sentence for both cases; the standing card's entry-term hints (inferred and plain) state the rule for Ph.D. records; `inferEntryTerm` in parse.ts takes the EARLIER of several admit terms unless a `masters` degree is dated between them (then the later, `how` says so), with the later term as `alternative`. Scenarios `phd-oce-clock-*`; `tests/background.test.ts`, `tests/transcript.test.ts`. Not changed: the other §4 clocks use the same entry term — open with the DGS (HANDBOOK-REVISIONS §10).
- **Registrar PDFs as fixtures, and what real geometry taught** (2026-09-26, later the same day). The DGS said to download the plan's public PDFs: 50 of 60 came down (the rest moved, bot-walled or not PDFs — the DECISIONS row lists them) and each ran through the app's OWN layout stage in node with `node --experimental-strip-types scripts/dev/pdf-to-lines.mts sample.pdf > lines.json` (pdfjs legacy build + `runsFromTextItems` / `runsToLines`, verbosity 0 so pdfjs's console warnings stay out of the JSON; PUBLIC documents only — `scripts/diagnose-transcript.mjs` is the tool for a real transcript, it prints shapes). Image-only PDFs: render pages with PyMuPDF at 200 dpi, then `node scripts/dev/ocr-lines.mjs out.json p1.png …` gives the OCR lines the browser would (tesseract.js + `public/ocr`, whitespace collapsed like `ocrPdfToLines`). The 34 `pdf-*` fixtures in `tests/fixtures/public-transcripts/` are those line lists; `sources.json` carries each PDF's URL, SHA-256, byte size and line count (the PDFs are NOT committed — re-download from the URL, check the hash, regenerate). `expected.json` gained `negative: true`: a key / legend / form / regulation must yield NO course row and its header fields are not compared. Two samples are pinned as read (ANU, the Vaasa template) — the ANU pin keeps two known artefacts, noted in DECISIONS. Parser changes the PDFs forced, all in `src/transcript/external.ts` unless said: `headerStraddles` in layout.ts vetoes a column split when one baseline holds a code/title header left of the gap and a credits/grade header right of it (ANU page 1); `CODE_STOPWORDS_RE` grew the function / document / address / money words, `proseSubject` refuses `in`/`on`/`for`/… only in prose case, digit-led codes need a capital in the title, `\d000-\d999` and a slash after a numeric code are refused; `courseLikeTitle` refuses program lines, e-mail lines, trailing commas and sentences (`PROSE_WORD_RE`, six words with two function words); the two-line continuation takes a numbers line only (≤ 3 wordy tokens); `LegendHints.hdScale` (an HD at a line end or “High Distinction”) makes `mapGrade` leave HD/D/CR/P/N raw, checked BEFORE the letter test; `scanWithMap` keeps the band beside the mark (`band`) and charges a one-digit “mark” after the credits beside an unmapped status one blank (`oneDigitMark`, credits-like column first — JNTU's marks-then-credits order is exempt); `readTermLine` joins “200 3”, knows `full year`, and gives a season-only line the current year. Tests: `tests/public-transcript-rules.test.ts` (four new cases), `tests/layout.test.ts` (the ANU page), `tests/public-transcripts.test.ts` (negatives).
