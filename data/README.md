...
| `counts_toward_mscse` | `yes` \| `no` \| `dgs_approval` | `dgs_approval` → the app counts it provisionally and tells the student to get sign-off — from the **ADGS**, who decides every requirement for MSCSE students (DGS 2026-09-11); the value name is historical. |
| `counts_toward_phd` | `yes` \| `no` \| `dgs_approval` | same, from the **DGS**, who decides for Ph.D. students. |
