...
| `active` | `yes` \| `no` | `yes` → shown in the student's course picker. `no` → hidden from the picker but still recognised when typed (old courses). Case does not matter (`No` is `no`, 2026-09-18). A retired course is NOT shown in the qualifying-examination cards or on the schedule cards (DGS, 2026-09-18): those list current courses. It stays in the table under “Include retired courses”, and the audit engine still counts it for a student who took it. A row that is retired AND marked as offered is reported. |
